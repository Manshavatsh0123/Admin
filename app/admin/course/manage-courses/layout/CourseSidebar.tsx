...
export default function CourseSidebar({ section, setSection }: any) {
    ...
    const Item = ({ id, label, icon: Icon }: any) => (
        <button
            ...
            className={`flex items-center gap-2 text-[16px] px-4 py-2 rounded-lg w-full text-left transition ${section === id ? "bg-[#BFE3DB] font-medium" : "hover:bg-gray-100"}`}>
            {Icon && <Icon size={18} />}
            {label}
        </button>
    )
    ...
    const SubItem = ({ id, label }: any) => (
        <button
            ...
            className={`flex items-center gap-2 pl-10 py-2 w-full text-left rounded-lg transition ${section === id ? "bg-[#BFE3DB]" : "hover:bg-gray-100"}`}>
            • {label}
        </button>
    )
    ...
    return (
        <div className="w-[200px] space-y-2">
            ...
            <button
                ...
                className={`flex items-center justify-between px-4 py-2 rounded-lg w-full ${curriculumOpen ? "bg-[#BFE3DB]" : "hover:bg-gray-100"}`}
            >
